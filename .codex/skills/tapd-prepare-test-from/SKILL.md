...
6. **Token Gate**：运行 `validate_environment_token.py`，直接读取选定环境中的 `authorization` 及可选的 `token_probe`、`login_url`、`account`、`password`。分支只由已校验的配置能力决定，禁止按环境名称、域名或业务码硬编码。外部请求最多尝试三次，每次失败记录脱敏的结构化 Warning；Token 失效且具备登录能力时使用 Playwright 自动续期，缺少安全续期能力、租户歧义或稳定定位信息时立即停止并请求用户处理。

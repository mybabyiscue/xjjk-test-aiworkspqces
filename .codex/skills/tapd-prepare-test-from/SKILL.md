...
* **Token 缓存反写**：`<skill_path>/assets/environments_config.json` 中的 `authorization` 字段。
...
* **环境与数据库配置绑定**：读取 `<skill_path>/assets/environments_config.json` （使用现有环境元数据配置字段：`name`, `login_url`, `api_domain`, `account`, `password`, `authorization`）和数据库配置管理技能。
* **平台选择与定向校验规范**：
  * 读取 `assets/environments_config.json` 中已注册的平台列表。
...
2. **加载环境配置文件**：优先加载工作区 `environments_config.json` 并提取所有注册的平台列表，重登时同步更新工作区与 `assets/environments_config.json`。
3. **平台选择与定向校验**：

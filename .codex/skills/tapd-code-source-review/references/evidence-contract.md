...
接口或符号命中只证明实现入口存在，不证明需求已正确实现。必须独立核对：

- 需求规则是否完整落地。
- 正常、异常和边界路径是否符合验收标准。
- 权限、状态流转、幂等和事务约束是否与需求一致。
- 持久化结果或外部依赖效果是否与预期一致。

每条用例必须归类为 `implemented`、`partially_implemented`、`not_implemented`、`implementation_conflict` 或 `unverifiable`。后四种状态均为 Halt 条件。

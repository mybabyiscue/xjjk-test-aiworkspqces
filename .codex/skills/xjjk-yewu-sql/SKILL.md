...
1. 当用户提供 JDBC URL、用户名、密码和连接名时，先在 `state/connections.json` 中按连接名匹配。
...
python scripts/metadata_cache.py upsert-connection --name "丝路测试" --jdbc "jdbc:mysql://192.168.70.89:3306" --username "silkroad" --password "83lchichucrodrl"
...
默认的业务领域别名配置放在 [business_domains.json](C:\Users\Administrator\.codex\skills\xjjk-yewu-sql\references\business_domains.json)。
如果要支持新的业务领域，应优先扩展该配置文件，而不是在响应层硬编码。

## 状态与配置

- 连接配置文件：[connections.json](C:\Users\Administrator\.codex\skills\xjjk-yewu-sql\state\connections.json)
...
- 缓存与文档模型说明：[cache_model.md](C:\Users\Administrator\.codex\skills\xjjk-yewu-sql\references\cache_model.md)
...
- 避免在 PowerShell 里临时写 `python -` 并直接夹带中文字面量做临时检查。在这个环境下，中文有可能在传给 Python 前被降成 `????`。更稳妥的方式是读取 UTF-8 文件，例如 `state/connections.json`，或者直接调用 skill 自带命令。

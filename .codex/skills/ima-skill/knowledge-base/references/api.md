...
The live `search_knowledge_base` response uses `kb_id` and `kb_name` for knowledge-base identity and display name. Do not assume the legacy `id` and `name` field names when matching or displaying results.
